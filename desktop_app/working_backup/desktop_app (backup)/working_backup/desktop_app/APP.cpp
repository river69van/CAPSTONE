...
//ig check kun masternode gud
bool master_node_check(NODES *check_the_node){

	if(check_the_node->MasterNode == true)
		return true;
	return false;
}
...
//to check if the buffer has a non int char
bool isValidInteger(const char* buffer_check) {

    if (buffer_check == nullptr || buffer_check[0] == '\0') {
        return false;  // Empty string or null pointer
    }

    int i = 0;
    if (buffer_check[i] == '+' || buffer_check[i] == '-') {
        i++;  // Allow leading '+' or '-'
    }

    for (; buffer_check[i] != '\0'; i++) {
        if (!isdigit(buffer_check[i])) {
...
void SerialReader(NODES* modifyNodes) {
    if (serial.FindWorkingCOMPort()) {
        ...
        while (serialActive) {
			...
			get_coordinates(serial, buffer, sizeof(buffer), serialActive, modifyNodes, run);


            if (buffer[0] != '\0') {

                imgX = modifyNodes->slave_cooordinates_x;
                imgY = modifyNodes->slave_cooordinates_x;
				
				
				if(modifyNodes->SlaveNode){
					printf("Status: slave node\n");
					printf("node number status: %d\n", modifyNodes->node_number_status);
					
				}else if(modifyNodes->trigger_flag){
					printf("Status: explosion");
					
				}else if(modifyNodes->MasterNode){
    ...
    switch (msg) {
        ...
        case WM_KEYDOWN:
            ...
            if (wParam == VK_SPACE && !serialActive) {
                ...
                std::thread(SerialReader, &node_object[1]).detach(); // Start serial reading in a new thread
            }
            switch (wParam) {
